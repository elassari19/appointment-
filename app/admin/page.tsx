...
const getStatusColor = (status: string) => {
  switch (status) {
    ...
    case 'On Leave': return 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400';
...
export default function AdminDashboardPage() {
  ...
  if (error) {
    return (
      <div className="p-8">
        <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg">Error loading analytics: {error}</div>
      </div>
    );
  }
    ...
    <div className="p-8">
      ...
      <div className="flex gap-4 mb-8 border-b border-border">
        <button onClick={() => setActiveTab('overview')} className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === 'overview' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}>
          <span className="material-icons-round text-sm mr-2 align-middle">dashboard</span>Overview
        </button>
        <button onClick={() => setActiveTab('monitoring')} className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors ${activeTab === 'monitoring' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}>
          <span className="material-icons-round text-sm mr-2 align-middle">analytics</span>Monitoring
        </button>
      </div>
        ...
        <>
          <div className="flex flex-wrap gap-3 mb-8">
            {filterPills.map((pill) => (
              <button key={pill.label} onClick={() => setActiveFilter(pill.label)} className={`px-5 py-2.5 rounded-full text-sm font-medium flex items-center gap-2 transition-all ${pill.label === activeFilter ? pill.color === 'primary' ? 'bg-primary text-primary-foreground' : pill.color === 'dark' ? 'bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900' : 'bg-card border border-border' : 'bg-card border border-border hover:bg-muted'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            {stats.map((stat) => (
              <div key={stat.label} className="bg-card p-6 rounded-3xl border border-border/60 shadow-sm transition-transform hover:-translate-y-1">
